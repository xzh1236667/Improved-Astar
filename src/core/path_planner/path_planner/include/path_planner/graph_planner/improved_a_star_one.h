 ...
 * ********************************************************
 */
#ifndef RMP_PATH_PLANNER_GRAPH_PLANNER_IMPROVED_A_STAR_ONE_H_
#define RMP_PATH_PLANNER_GRAPH_PLANNER_IMPROVED_A_STAR_ONE_H_
 ...
 */
class ImprovedAStarPathPlanner1 : public PathPlanner
{
public:
  using Node = rmp::common::structure::Node<double>;
  class ImprovedNode1 : public Node
  {
  public:
  ...
  struct compare_cost
  {
     ...
     */
    bool operator()(const ImprovedNode1* n1, const ImprovedNode1* n2) const
    {
     ...
     */
    ImprovedNode1(double x = 0, double y = 0, double theta = 0,int accumulate_swing=0,double with_parent_yaw_dif=0,double g = 0.0, double h = 0.0, int id = 0, int pid = 0
               );
    double theta() const;
    ...
    bool operator==(const ImprovedNode1& n) const;
   ...
   */
  ImprovedAStarPathPlanner1(costmap_2d::Costmap2DROS* costmap_ros);
  
  /**
   * @brief Destory the Improved A* object
   */
  ~ImprovedAStarPathPlanner1() = default;
  ...
  void cal_new_node(ImprovedNode1* current,ImprovedNode1* newNode,double steer);
  void computeCollisionIndexes(int theta_index, std::vector<IndexXY> & indexes_2d);
  int getIndex(const double yaw);
  bool detectCollision(const int & theta_index,ImprovedNode1* Node);
  double cal_gcost(ImprovedNode1* current, ImprovedNode1* new_node,const ImprovedNode1& goal_node);
  void exampleFunction();
  int _worldToIndex_1(double wx, double wy,double theta);
  double calh_cost(ImprovedNode1* new_node,const ImprovedNode1& goal_node);
protected:
  /**
   ...
   */
  std::vector<ImprovedNode1> _convertClosedListToPath(std::unordered_map<int, ImprovedNode1*>& closed_list,
                                                   const ImprovedNode1& start, ImprovedNode1* current);
