...
public:
                                ...
                                const ros::Publisher& publisher, const std::string& frame_id)
  {
    ...
    {
      count++;
      grid.data[grid2index(pt)] =150;
    }
    ROS_INFO("Number of points in expand zone: %d", count);
    publisher.publish(grid);
  }
 template <typename Points>
  static void publishExpandZone_1(const Points& expand, const costmap_2d::Costmap2D* costmap,
                                const ros::Publisher& publisher, const std::string& frame_id)
  {
    nav_msgs::OccupancyGrid grid;
    float resolution = costmap->getResolution();
    ...
    grid.info.origin.orientation.w = 1.0;
    grid.data.resize(grid.info.width * grid.info.height);
    int length=static_cast<int>(grid.info.width * grid.info.height);
    int sum[length]={0};
    for (unsigned int i = 0; i < grid.data.size(); i++)
    {
      grid.data[i] = 0;
    ...
    auto grid2index = [&](const Point3d& pt) {
      unsigned int mx,my;
      costmap->worldToMap(pt.x(),pt.y(),mx,my);
      return static_cast<int>(mx) + static_cast<int>(grid.info.width * my);
    };
    int count=0;
    for (const auto& pt : expand)
    {
    ...
    ROS_INFO("Number of points in expand zone_improved_astar: %d", count);
    publisher.publish(grid);
  }
  /**
